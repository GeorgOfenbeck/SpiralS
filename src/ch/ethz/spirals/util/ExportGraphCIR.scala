...
import scala.virtualization.lms.internal.{GraphTraversal, NestedBlockTraversal}

import collection.mutable
import java.io.PrintWriter

trait ExportGraphCIR extends GraphTraversal  {  self =>
  ...
  def emitNode(sym: Sym[Any], rhs: Def[Any])(implicit stream: PrintWriter) = rhs match {
    ...
    case _ => {
      ...
      val (sign, fillcolor, fontcolor, fontsize, shape) = rhs match {
        ...
        case Reflect(ForLoop(start, end, i, body), _, _) => ("ForLoop[" + i.toString + " , " + end.toString + "]", "\"#660033\"", "white", 24.0, "record")
  ...
  def emitClusterNodes (start: Exp[Any], stream: PrintWriter) {
    ...
    val emitter = new NestedBlockTraversal {
      ...
      override def traverseStm(stm: Stm): Unit = stm match {
        case TP(s, d@Reflect(ForLoop(start, end, i, body), _, _)) => {
  ...
  def exportGraph[A, B](f: Exp[A] => Exp[B], file: String, landscape: Boolean, title: String) (implicit mA: Manifest[A], mB: Manifest[B]) : Unit = {
    ...
    exportGraph[B]((sList, IR.CIR_Optimizer.reifyBlock(f(s))), file, landscape, title)
  }

  def exportGraph[B](block: (List[Sym[Any]], Block[B]), file: String, landscape: Boolean, title: String) (implicit mList: List[Manifest[Any]], mB: Manifest[B]) : Unit = {
    constants = CIR_Optimizer.getConstants(block)
    val opsCount = CIR_Optimizer.opsCount(block)
    this.adds = opsCount._1
    this.muls = opsCount._2
