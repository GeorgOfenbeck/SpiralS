...
class ICC extends Compiler {

  val compilerName = "icc"

  override val shared = getOS() match {
    case "Windows" => "shared"
    case "Mac" => "dynamiclib"
    case "Unix" => "shared"
    case _ => throw new CCompilerException("Can not determine dynamic lib option")
  }

  def getCompilerExec () = getOS match {
    case "Windows" => findExec("icc.exe")
    case _ => findExec("icc")
  }
}
