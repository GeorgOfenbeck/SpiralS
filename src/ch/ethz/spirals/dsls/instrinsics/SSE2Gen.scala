...
package ch.ethz.spirals.dsls.instrinsics
...
trait SSE2Gen extends SSEGen {
  ...
  override def emit_vstore [T] (sym: Sym[_], a: Rep[Array[T]], pos: Rep[Int], v: Packed[T]) : Unit = v.m.toString match {
    case "Integer" => stream.println("_mm_store_si128(" + quote(a) + " + " + quote(pos) + ", " + quote(v.getIRep()) + ");")
    case "Long"    => stream.println("_mm_store_si128(" + quote(a) + " + " + quote(pos) + ", " + quote(v.getIRep()) + ");")
  ...
  override def emit_vload [T] (v: Packed[_], a: Rep[Array[T]], pos: Rep[Int], size: Int) : Unit = v.m.toString match {
    case "Integer" => emitVectorDef(v.getIRep(), v.m, "_mm_loadu_si128(" + quote(a) + " + " + quote(pos) + ")")
    case "Long"    => emitVectorDef(v.getIRep(), v.m, "_mm_loadu_si128(" + quote(a) + " + " + quote(pos) + ")")
  ...
  override def emit_vadd [T] (v: Packed[_], a: Packed[T], b: Packed[T]) : Unit = a.m.toString match {
    case "Integer" => emitVectorDef(v.getIRep(), a.m, "_mm_add_epi32(" + quote(a.getIRep()) + ", " + quote(b.getIRep()) + ")")
  ...
  override def emit_vsub [T] (v: Packed[_], a: Packed[T], b: Packed[T]) : Unit = a.m.toString match {
    case "Integer" => emitVectorDef(v.getIRep(), a.m, "_mm_sub_epi32(" + quote(a.getIRep()) + ", " + quote(b.getIRep()) + ")")
