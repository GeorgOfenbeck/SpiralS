...
import scala.virtualization.lms.common.{CLikeGenArrayOps, CGenBase}
...
trait CGenArrayOpsExt extends CGenBase with CLikeGenArrayOps with CUnparser { self =>
  ...
  override def emitGlobalNodes (block: Block[Any]) = {
    ...
    val globalArraysTraversal = new NestedBlockTraversal {
      val IR = self.IR
      import IR._
      override def traverseStm(stm: this.IR.Stm): Unit = stm match {
        case TP(s, Reflect(GlobalConstArrayNew (values), _, _)) => {
          stream.print("const " + getArrayInnerTypeManifest(s.tp) + " " + quote(s.asInstanceOf[CGenArrayOpsExt.this.IR.Exp[Any]]) + "[] = { ");
          stream.print(values.head)
          values.tail.foreach ( v => stream.print(", " + quote(Const(v).asInstanceOf[CGenArrayOpsExt.this.IR.Exp[Any]]) ) )
          stream.println(" };")
          super.traverseStm(stm.asInstanceOf[this.IR.Stm])
        }
        case _ => {
          super.traverseStm(stm.asInstanceOf[this.IR.Stm])
        }
    ...
    globalArraysTraversal.traverseBlock[Any](block.asInstanceOf[globalArraysTraversal.Block[Any]])
  }

  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = {
    rhs match {
      case GlobalConstArrayNew (_) => // do nothing
